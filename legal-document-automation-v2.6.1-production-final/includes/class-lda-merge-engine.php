...
class LDA_MergeEngine {
    ...
    public function mergeDocument($template_path, $merge_data, $output_path) {
        try {
            ...
            // Use the original Simple DOCX processing (which worked for multi-page)
            LDA_Logger::log("Using original LDA_SimpleDOCX processor");
    ...
    private function applyFieldMappingsDirectly($merge_data) {
        ...
        LDA_Logger::log("Merge data keys: " . implode(', ', array_keys($merge_data)));
        LDA_Logger::log("Merge data sample: " . json_encode(array_slice($merge_data, 0, 5), JSON_PRETTY_PRINT));
        ...
        LDA_Logger::log("Form ID: " . $form_id);
        ...
        LDA_Logger::log("Form mappings: " . json_encode($form_mappings, JSON_PRETTY_PRINT));
        
        // Apply field mappings
        foreach ($form_mappings as $merge_tag => $field_id) {
            $field_value = isset($merge_data[$field_id]) ? $merge_data[$field_id] : '';
            LDA_Logger::log("DEBUG: Checking field {$field_id} for merge tag {$merge_tag}");
            LDA_Logger::log("DEBUG: Field {$field_id} value: '{$field_value}'");
            LDA_Logger::log("DEBUG: Available merge data keys: " . implode(', ', array_keys($merge_data)));
    ...
    /**
     * Apply field mappings to merge data
     */
    private function applyFieldMappings($merge_data) {
        LDA_Logger::log("=== APPLYING FIELD MAPPINGS ===");
        LDA_Logger::log("Merge data keys: " . implode(', ', array_keys($merge_data)));
        LDA_Logger::log("Merge data sample: " . json_encode(array_slice($merge_data, 0, 5), JSON_PRETTY_PRINT));
